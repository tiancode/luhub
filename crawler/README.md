...
`python -m crawler.run` 向 **stdout** 按 **NDJSON** 打印——**每行一份** maccms
响应：`{ code, msg, page, pagecount, class:[{type_id,type_name}], list:[MaccmsVod] }`，
每产出一条即 flush。适配器可逐页产出(maccms/mocksite)或**逐部产出**(yhdm:一行一部番)。
TS 桥接 `src/collect/python.ts` 逐行接收、**收到即 `ingestResponse` 入库**——
中途断开时已写出的行已落库,不会全部丢失。诊断信息一律走 **stderr**，不污染 stdout 的 JSON。

### 断点续采(跳过已采集)

`syncViaPython` 入库前会取该源已入库的 `sourceVodId`,写临时文件经 `--seen-file`
传给 Python;适配器在抓详情页**之前**跳过这些 id(见 `yhdm` 的 `crawl()`)。
默认开启;`pnpm collect:py ... --no-resume` 可强制重新全量抓取。增量(`--hours`)
模式不跳过,以便刷新已有条目的新集。
...
- 后台 `/admin` 的「采集」已接通 Python 适配器:`kind=html` 且设置了 **Python 适配器名**
  (如 `yhdm`)的源,采集时自动走 `syncViaPython`(流式边抓边入库 + 断点续采);其余源仍走
  TS 原生 maccms(`syncSource`)。分发逻辑在 `src/lib/admin/collect.ts`。
  - 在 `/admin/sources` 新增/编辑源:类型选 `html`、「Python 适配器」填 `yhdm`、地址填站点根
    (如 `https://yhdm.one`)。采集记录里能看到逐部 `+入库 …` 进度。
  - **服务端需能跑爬虫**:进程会 spawn `PYTHON_BIN`(默认 `python3`)执行 `python -m crawler.run`,
    该解释器需装好 `crawler/requirements.txt`(httpx/bs4)。建议在 `.env` 里设
    `PYTHON_BIN` 指向带依赖的解释器(如 `crawler/.venv/bin/python` 的绝对路径)。
