...
**收藏 / 观看历史**(无登录):靠 `src/lib/visitor.ts` 给每个浏览器发的随机匿名 ID(cookie `luhub_vid`)定位访客——不是真浏览器指纹(那会撞车、易失效)。`getVisitorId()` 只读(渲染期),`ensureVisitorId()` 才种 cookie(只能在 Server Action 里)。模型 `Favorite` / `WatchHistory` 均按 `@@unique([visitorId, videoId])` 隔离;读写在 `src/lib/library/`(`queries.ts` + 公开 `actions.ts`,均校验 `videoId` 真实存在防伪造)。`Player.tsx` 节流上报进度并据 `WatchHistory` 续播;`/me` 是「我的」页(历史+收藏)。
